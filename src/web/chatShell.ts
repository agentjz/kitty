...
export class WebChatShell implements InteractionShell {
  ...
  readonly output = {
    plain: (text: string): void => this.broadcast({ type: "user", text }),
  ...
  attach(server: WebSocketServer, replay: (send: (event: WebEvent) => void) => Promise<void>): void {
    server.on("connection", (socket) => {
      this.clients.add(socket);
      this.send(socket, { type: "presentation", labels: this.labels });
      void replay((event) => this.send(socket, event));
  ...
  replaySession(session: SessionRecord, send: (event: WebEvent) => void): void {
    for (const message of session.messages) {
      ...
      if (message.content && !message.tool_calls?.length) send({ type: "message", text: message.content });
    }
  }
