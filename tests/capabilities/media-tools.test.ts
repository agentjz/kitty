...
import { createMediaTools } from "../../src/capabilities/tools/media/index.js";
...
test("media capability exposes image and mixed-result video tools", () => {
...
test("generate_video persists video_id, rate-limits early polls, then saves completion", async (t) => {
  ...
  const recordPath = path.join(getProjectStatePaths(root).capabilitiesDir, "media", "video-tasks", "video_acceptance.json");
