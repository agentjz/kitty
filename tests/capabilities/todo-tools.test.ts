...
test("todo capability writes session todo facts for working memory", async (t) => {
  const root = await createTempWorkspace("todo-capability", t);
  const context = createToolContext(root);
  const registry = await createDefaultAgentToolRegistry(context.config);
  assert.deepEqual(
    (registry.entries ?? [])
      .filter((entry) => entry.origin.sourceId === "capability:todo")
