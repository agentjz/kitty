...
`subagent` 是聚焦 agent execution 工具集合。它把子执行写入 control-plane，支持启动和检查。subagent execution 默认带阻塞型 `waitPolicy`；lead 启动后让出当前轮，host 等 subagent 结束后用 internal wake 事实恢复 lead。worker 的最终可见回答写回 execution，lead 醒来时能看到实际结论。

`team` 是 teammate execution、成员和消息工具集合。成员、消息和队友执行都进入 control-plane。team execution 默认带阻塞型 `waitPolicy`；lead 启动 teammate 后让出当前轮，worker 完成后 teammate 状态回到 idle，最终可见回答写回 execution。

`skills` 是项目运行时 skill 工具集合。它发现项目 `SKILL.md`、`.skills/**/SKILL.md` 和 `skills/**/SKILL.md`，只把名称、说明、路径和资源索引放进运行事实。模型认为当前任务需要某个方法时，显式调用 `skill_load` 读取完整正文；需要 skill 包内资料、脚本、示例或资产时，显式调用 `skill_read_resource` 读取。机器不做关键词匹配、语义路由或自动加载。
