...
`kitty eval` 是真实体验验收入口。它列出简单问题、长会话、历史隔离、项目地图、memory、background、subagent 的验收场景和机器事实。`kitty eval --run` 运行本地可验证检查，并用假 provider 跑真实 host turn golden 场景，输出 pass/fail/skip，不替模型打分。
