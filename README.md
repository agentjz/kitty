...
Kitty 内置的模型都是免费的，并且我会到处寻找各种免费的大模型并加入配置。具体可用地区、RPM、TPM、RPD、模型权限和免费政策由 Provider 的账号与项目决定，这个可能变化。但免费就是基本的原则：无论是日常使用还是高强度开发，一切都应该是免费的！我正在做一个免费大模型的api中转站，把这些免费的东西尽可能一网打尽。

Kitty 已内置 LLM2API 中转站预设。LLM2API 在本机运行时，选择该预设，填写它签发的下游 API 密钥，并把模型填成 `/v1/models` 返回的模型 id；Kitty 继续使用统一 Chat Completions 协议和网关公开能力，上游资源池、上游 API Key 分配与厂商 wire 差异由中转站统一处理。
