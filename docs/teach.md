...
子代理是另一个拥有独立上下文的 agent。它把专项调查、大量文件和中间过程留在自己的上下文里，最后只把结论交回主 agent。这样主 agent 的上下文可以继续保留整个任务的目标、关键事实和决策。

历史压缩整理同一段会话里较早的信息；子代理隔离一块独立工作的上下文。两者分别处理时间方向和任务分支带来的上下文压力。

主 agent 给子代理一个明确目标、边界和期望结果；子代理带着自己的上下文和工具完成这块工作，再把压缩后的结果交回主 agent。

### 具体例子

主 agent 正在完成一个复杂任务，同时需要有人专门查看相关文件。它可以交给子代理：

```text
目标：查看相关文件，找出需要同步调整的地方。
边界：只研究测试，不修改产品代码。
期望结果：列出需要修改的测试文件和原因。
```

Kitty 为子代理启动独立的 agent 循环。查看文件、工具结果和中间判断都留在子代理上下文；子代理完成后，把“需要修改的测试文件和原因”交回主 agent，再由主 agent 完成整个任务。
...
用户还要确认客人有没有额外忌口。主 agent 把这件独立的小事交给子代理：
...
偏好资料、工具结果和判断过程留在子代理的独立上下文。子代理完成后，只把“没有额外忌口”交回主 agent。
...
```text
你是本 session 的主导 agent。你是一位重视结构和事实的工程师，表达短、准、清楚。现在要做西红柿炒鸡蛋，食材清单已经确认有 3 个西红柿、4 个鸡蛋，用户喜欢鸡蛋炒嫩一点。项目长期规则要求先看事实，再做判断，再行动。已经加载的西红柿炒鸡蛋 skill 说明：鸡蛋先炒到刚凝固，西红柿炒出汁后，再把鸡蛋倒回锅里。子代理已经确认客人没有额外忌口。你可以用 read 读取资料，用 skill_load 加载熟练做法。现在用户说：鸡蛋再嫩一点，少放糖。
```
