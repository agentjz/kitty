...
## 阶段 25：宣传站反复推翻复杂结构，最后回到空间、自然流和单一事实

时间：2026-07-18

版本线：从 4.0.5 进入 4.0.6 官网改版；改版前 TUI GIF 已由 `5cd7bf1` 提交并推送。

这一阶段要解决的不是“再写一篇智能体教程”，而是让官网先像一个正在快速演进的开源产品，再承接原有教学正文。首屏需要明确出现 Kitty Agent、当前 npm 版本、安装、源码和真实 TUI；全站需要与 Kitty TUI 和 `kitty start` 使用同一套深色视觉语言。

### 被推翻的宣传结构

第一轮把卖点拆成 `WHY KITTY` 功能格子和 `OPEN BY DEFAULT` 双栏事实区，还增加了 K 图标、英文标签和大量卡片。这条路线被真实验收否决：它像通用 SaaS 模板，宣传内容尴尬，也重新出现了已经明确禁止的 sidebar 感。

后续又先后尝试过：

- 上黑下白的分段页面，品牌与教学视觉断裂；
- 大圆轨道、均匀步骤节点、盘子插画和 Runtime Dock 式组件，像课程图解，不像真实产品；
- 代码块左侧强调线和低对比深色文字，既像侧栏又难读；
- 把循环状态拆成左右两列，思考在左、工具在右，阅读顺序不成立；
- 为了固定 slot 位置写八条固定网格轨道，思考增长后覆盖结果、计划和后台；
- 用多条横线区分循环状态，错位时界面更加割裂；
- 把 `Kitty` 与 `Agent` 写成两个独立 `<pre>` 再靠 flex gap 拼接，断点越多，字符宽度和间距越难同时成立；
- 合并字标后又给 identity 固定 630px，容器虽然居中，实际更窄的左对齐字符画仍然视觉偏左；同时把桌面字号压到 10px，失去 `kitty start` 的原有体量；
- 把仓库版本 `4.0.5` 直接硬编码到官网，混淆当前工作树版本与 npm `latest` 发布事实；
- 把一段完整模型输入放进横向滚动的 `<pre>` 文本框，正文被切成独立滚动区域。

这些尝试有一个共同问题：为了控制外观，增加了第二套脆弱几何。内容一变、窗口一缩、文字一长，就需要继续补行高、断点、隐藏规则和局部特判。

### 最后收敛的首屏

版本 presentation 改为读取 npm registry 的 `@jun133/kitty/latest`；读取失败只显示 `latest`，不伪造版本号。品牌字标继续使用 Kitty 的 ANSI Compact 事实，但由一个完整三行字符画 owner 呈现，不再拼两个块。

只合并字标仍然不够。`kitty start` 的真实结构是独立品牌 banner：桌面最小高度 240px，移动端 190px，版本与字标作为一个整体垂直居中。宣传页最终复用这条结构边界；identity 由字标内容宽度决定并整体水平居中，桌面字号恢复为同源的 14px，版本继续与字标左边缘对齐。主张、摘要、按钮、安装命令和 GIF 位于 banner 之后的独立内容区，不再通过压缩字标和间距把所有东西塞进同一个容器。banner 已经拥有底部留白，后续内容区只保留普通顶部间距，不能再叠加第二份大留白。

GIF 保留真实 TUI 资源并缩小展示，不增加“真实运行界面”标题条。顶部 GitHub 图标保持普通图标，不加边框容器。全站使用同一深色主干，不恢复欢迎语、尾部大 CTA、功能卡片阵列或额外设计哲学章节。

### 最后收敛的流式演示

西红柿炒鸡蛋继续作为完整任务例子。八步、逐字输出、播放、暂停、复制和 reduced-motion 合同保留，状态仍由 `site/script.js` 的前序快照和当前更新驱动。

HTML 只声明一组稳定 slot：任务、追加要求、思考、结果、计划、后台、工具和回答。CSS 不再给它们分配固定网格轨道，也不画内部横向分割线。非空内容按自然文档流向下生长，空 slot 不占视觉空间，长内容由输出台现有纵向滚动承接。思考只会把后续事实向下推，不能覆盖它们；工具保持左对齐并位于最终回答之前。

完整模型输入也回到普通正文流。复制按钮使用 `data-copy-target` 读取正文节点的 `textContent`，长文本只有一个 owner；它不再被重复硬编码进按钮属性，也不再产生横向滚动条。
...
- 不把正文放进独立横向滚动区；
- 不为了“固定位置”牺牲自然阅读顺序；
- 不用装饰线、卡片和状态 dock 代替清楚层级；
- 先复用 Kitty 当前真实界面的结构事实，再决定宣传页如何表达；
- 结构、尺寸、不溢出和交互由机器检查，最终观感仍由项目所有者在浏览器验收。

本轮静态检查确认：品牌 banner 由一个字标 owner 呈现，桌面/移动端高度为 240px/190px，桌面字号 14px并按内容宽度居中；循环没有固定轨道和内部横线；完整模型输入是无框正文并从目标节点复制。仓库完整验证为 384 项测试、383 通过、0 失败、1 项平台跳过，构建与发布包检查通过。

## 阶段 26：官网首屏收敛为单一 Hero 标题
