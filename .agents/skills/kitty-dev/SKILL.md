---
name: kitty-dev
description: 维护 kitty 项目时使用。适用于修改实现、接口、数据、测试、事实文档、依赖、构建和运行配置，要求先确认事实，再完成实现、验证、文档同步与交付闭环。
---
...
- 每次任务完成并通过必要验证后，直接提交并推送当前分支，不等待 owner 另行提醒。提交前确认暂存区只包含当前任务文件；工作区中与当前任务无关的改动不得暂存、回滚或覆盖。推送失败时保留本地提交并明确报告原因，不无限重试。部署和发布仍只在 owner 明确授权时执行。
