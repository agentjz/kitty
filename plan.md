...
第二类是本机 llama.cpp 文本推理。用户应能在现有“语言模型设置”中选择本地 Provider 和已安装模型，复用 Kitty 的同一 Agent、session、tool、streaming、context 和恢复链路。部署应匹配当前电脑的 8 GB RTX 5070 Laptop GPU、32 GB 内存和 D 盘空间；当前先只保留一个可验证的 Google Gemma 3 本地模型。Google Gemini 没有可下载的开放权重；本地 Google 模型使用官方开放权重 Gemma 3。

本次交付包含：Agnes 图片/视频 Provider 适配、两个 Agent 工具、运行时 Agnes 多模态 skill、Web 配置与人类生成工作台、llama.cpp Provider 适配、D 盘二进制与 Gemma GGUF 模型部署、真实图片/视频/本地推理验收、边界测试和 `spec.md` 同步。commit/push 不在未明确授权的范围内。
...
- 当前本地模型集合只保留 Google `gemma-3-12b-it-q4_0`（官方 QAT，8,074,473,920 字节，ModelScope 官方镜像），文件已完整合并到 `D:\AI\llama.cpp\models`，SHA-256 marker 已生成并验证；实际 SHA-256 为 `dd53172ff3a7b1b16c8fb3d944b87f42a6228ff2de3825b8813ae90d988434cd`。Kimi、MiniMax、GLM、DeepSeek 和 Phi 不在当前 catalog、preset 或部署清单中。
...
- llama.cpp Provider/model preset、本机 CUDA binaries、Gemma 模型下载/校验清单和 router 启停状态脚本已完成；Gemma GGUF 已达到 manifest 精确字节数并通过 SHA-256 marker 校验，真实推理证据仍待本机 router 启动后完成。
...
12. 本机 `llama-server`、Gemma GGUF、SHA-256、router 加载、32K context、流式输出和工具调用尚未真实验证。
...
- 语言模型主链路新增一个明确的 `llama.cpp` Provider，认证模式为 `none`，当前只暴露 Gemma preset，复用 Chat Completions streaming、tool call、tool result replay 和 session 恢复。
...
- Web 的“图像与视频”工作流同时承担配置与人类生成：图片表单支持提示词、清晰度和比例；视频表单支持提示词、画幅、时长和负面提示词，创建后自动低频查询并展示 `video_id`、状态、进度、预览和项目内路径。语言模型页显示唯一的 Gemma 本地 preset。
...
- [ ] 用 Kitty 对 Gemma 本地模型验收流式回复、中文、上下文配置、真实 tool call、无 key 和错误模型边界；记录加载时间、速度与内存，并根据实测修正 catalog/参数。
